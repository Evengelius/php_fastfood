...
if (isset($_SESSION['users'])) {
    ...
    if ($address->paye == 0) {
        if ($subject == 'cart') {
            $cart_result = $dbh->prepare("SELECT * FROM commande_burger LEFT JOIN burger b on b.id = commande_burger.burger_id WHERE burger_id = :burgerId AND commande_id = :user_id");
            $cart_result->execute([
                'burgerId' => $idBurger,
